...
class ChartZoomer(var zoomType: ZoomType?) {
    ...
    fun computeZoom(computator: ChartComputator): Boolean {
        if (zoomer.computeZoom()) {
            ...
            val pointWithinViewportX =
                (zoomFocalPoint.x - scrollerStartViewport.left) / scrollerStartViewport.width()
            val pointWithinViewportY =
                (zoomFocalPoint.y - scrollerStartViewport.bottom) / scrollerStartViewport.height()
    ...
    private fun setCurrentViewport(
        ...
        when (zoomType ?: return) {
            ZoomType.HORIZONTAL -> computator.setCurrentViewport(
                left,
                currentViewport.top,
                right,
                currentViewport.bottom
            )
            ZoomType.VERTICAL -> computator.setCurrentViewport(
                currentViewport.left,
                top,
                currentViewport.right,
                bottom
            )
            ZoomType.HORIZONTAL_AND_VERTICAL -> computator.setCurrentViewport(
                left,
                top,
                right,
                bottom
